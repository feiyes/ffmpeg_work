...
#ifdef RENDER_VIDEO
void render_video(SDL_Renderer* renderer, SDL_Texture* texture, SDL_Rect* rect,
                  AVFrame *frame, struct SwsContext *sws_ctx, AVFrame* sws_frame)
{
#ifdef SCALE_VIDEO
    sws_scale(sws_ctx, (const unsigned char* const*)frame->data, frame->linesize,
...
static void decode_process(AVCodecContext* codec_ctx, AVPacket* packet, AVFrame* frame, ff_render_info* render, enum AVMediaType type)
{
    int ret = -1;
...
#ifdef RENDER_VIDEO
            render_video(render->renderer, render->texture, &render->rect, frame, render->sws_ctx, render->sws_frame);
#endif
...
int main(int argc, char* argv[])
{
    ...
    if (!render.screen) {
        log_warn("SDL_CreateWindow failed, error(%s)\n", SDL_GetError());
        return -1;
    }

    render.renderer = SDL_CreateRenderer(render.screen, -1, 0);
    if (!render.renderer) {
        log_warn("SDL_CreateRenderer failed, error(%s)\n", SDL_GetError());
        return -1;
    }

    render.texture = SDL_CreateTexture(render.renderer, SDL_PIXELFORMAT_IYUV,
                                       SDL_TEXTUREACCESS_STREAMING,
                                       video_ctx->width, video_ctx->height);
    if (!render.texture) {
        log_warn("SDL_CreateTexture failed, error(%s)\n", SDL_GetError());
        return -1;
    }
#endif

#ifdef SCALE_VIDEO
    render.sws_frame = av_frame_alloc();
    if (!render.sws_frame) {
        log_err("av_frame_alloc failed\n");
        return -1;
    }

    render.sws_frame->width  = video_ctx->width;
    render.sws_frame->height = video_ctx->height;
    render.sws_frame->format = video_ctx->pix_fmt;
    ret = av_frame_get_buffer(render.sws_frame, 0);
    if (ret < 0) {
        log_err("av_frame_get_buffer failed, error(%s)", av_err2str(ret));
        return -1;
    }

    render.sws_ctx = sws_getContext(video_ctx->width, video_ctx->height, video_ctx->pix_fmt,
        ...
        if (event.type == REFRESH_EVENT) {
            ...
            if (ret >= 0) {
                if (packet->stream_index == video_index) {
                    decode_process(video_ctx, packet, frame, &render, AVMEDIA_TYPE_VIDEO);
                } else if (packet->stream_index == audio_index) {
                    decode_process(audio_ctx, packet, frame, &render, AVMEDIA_TYPE_AUDIO);
                }

                av_packet_unref(packet);
            } else {
                if (video_ctx) decode_process(video_ctx, NULL, frame, &render, AVMEDIA_TYPE_VIDEO);
                if (audio_ctx) decode_process(audio_ctx, NULL, frame, &render, AVMEDIA_TYPE_AUDIO);
            }
    ...
    SDL_WaitThread(refresh_thread, NULL);
#ifdef RENDER_VIDEO
    SDL_Quit();
#endif

#ifdef SCALE_VIDEO
    av_frame_free(&render.sws_frame);
    sws_freeContext(render.sws_ctx);
#endif

...
int main(int argc, char** argv)
{
    ...
    const char* out_file = "800x450_crop_vfilter.yuv";
    ...
    ret = avfilter_graph_create_filter(&sink_ctx, dst_buf, "out", NULL, NULL, filter_graph);
    if (ret < 0) {
        log_err("avfilter_graph_create_filter failed, error(%s)\n", av_err2str(ret));
        return -1;
    }
    ...
    if (ret < 0) {
        log_err("av_opt_set_int_list failed, error(%s)\n", av_err2str(ret));
        return -1;
    }
